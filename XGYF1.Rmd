...
ggplot(Q3, aes(x=age_cs, y=riskSR, color=sex)) +
  ...
  scale_color_manual(labels=c("Female", "Male"), values=c("#E69F44", "#52B4E9")) + 
  labs(x="Age Centered", y="Self Reported Risk")
```

Q5. What does the graph suggest about risk attitudes
```{r q05}
#the age negatively predict self-report risk attitude among male
```
...
summary(mod2) 
#yes it does as the adjusted R2 increased
```
...
```{r q09}
combi %>% group_by(trialtype) %>% summarise(prob=mean(RiskyChoice))
```

Q10. How do you interpret this in words?
```{r q10}
#participants took the riskier choice 47% of the time in the normal conditions and 38% of the time under high risk conditions 
