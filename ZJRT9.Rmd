...
#DV=risk attitude
#Random Effect=Participant's id
...
#sex must be categorical and its binary instead.
```
...
ggplot(self, aes(x=age_cs,y=riskSR,colour= as.factor(sex))) +
 geom_smooth(method=lm) + 
 geom_point()+scale_colour_manual(name="sex", values = c("light blue", "light pink"), labels=c("male", "female"))
```

Q5. What does the graph suggest about risk attitudes
```{r q05}
#A negative relationship, while males had higher attitude in lower ages, this decreases with age in a similar level to females
```
...
#yes because it now predicts 2% of the values as opposed to 0.07%
...
```{r q09}
combi_p<-combi%>% group_by(trialtype, RiskyChoice)%>% count()%>% mutate(prop=sum(n)/100)
```

Q10. How do you interpret this in words?
```{r q10}
#Participants are less likely to make the riskier choice in high risk situations
```
...
```{r q12}
mod4 <- lmer(RiskyChoice ~ trialtype + (1+ partid)+(1|scenario)+(1|partid), data=combi)
summary(mod4)
```
