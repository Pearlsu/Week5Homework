...
```{r q01}
#glmm, dv: RiskSR, random effect: age, participant
```
...
```{r q04}
ggplot(self, aes(x = age, y = riskSR, color = sex)) + geom_point() + geom_smooth(method = "lm") + scale_fill_discrete(name = "Sex", labels = c("male", "female"))
```

Q5. What does the graph suggest about risk attitudes
```{r q05}
#there is a minor negative correlation between age and self-reported risk attitude in male, but there is no correlation between that in female
```
...
```{r q06}
self$partid <-  as.factor(self$partid)
...
```{r q09}
sum <- gambles %>% group_by( trialtype) %>% summarise(prop = mean(RiskyChoice))
#normal: 48.0%, high risk: 38.7%
```

Q10. How do you interpret this in words?
```{r q10}
#people are more likely to make riskier choices in normal situations than in risker situations
```
...
```{r q12}
mod4 <- lmer(RiskyChoice ~ trialtype + (1|partid) + (1 + trialtype |partid) + (1|scenario), control = lmerControl(optimizer="bobyqa") ,data = gambles)
```
