...
#dv: self reported risk attitude, riskSR, pseudo-continuous numerical
#random effect of sex and partid
```
...
```{r q02}
self <- self %>% 
  mutate(age_cs = (age-mean(age))/sd(age))
```
...
```{r q03}
#sex needs to be factor
self <- self %>% mutate(sex = as.factor(sex))
self %>% 
  ggplot(aes(x = age_cs, y = riskSR, colour = sex)) + 
  geom_point() + 
...
#confused where aes comes in
```

Q5. What does the graph suggest about risk attitudes
```{r q05}
#males have higher risk attitudes compared to females at lower age. As age increases, female risk attitudes barely change but male risk attitudes decreases such that males and females converge to similar risk attitudes
```
...
#lmer is for random effects. No random effects in this model so lm used instead
...
#question did not specific random or fixed effect of age, so I assumed fixed effect to allow comparison by anova. Or else could compare with variance when comparing lm and lmer
summary(mod2)
anova(mod1, mod2)
```
...
#riskier choice under normal conditions 47.98%
#riskier choice under high risk conditions 38.69%
```
...
```{r q11}
#consider random effects of sex, participant, risk attitude and scenario of gamble
```
